...
@router.put("/{item_id}", response_model=TrainingExample)
async def update_training(item_id: str, item: TrainingUpdate):
    """Update existing training example"""
    try:
        if item.tone and item.tone not in VALID_TONES:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid tone. Must be one of: {', '.join(VALID_TONES)}"
            )
        
        result = db_direct.update_training(
            item_id,
