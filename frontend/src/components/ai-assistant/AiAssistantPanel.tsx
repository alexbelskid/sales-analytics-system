...
export default function AiAssistantPanel() {
    ...
    return (
        <div className="h-full flex flex-col relative overflow-hidden">
            ...
            <div className={cn(
                "transition-all duration-700 ease-in-out overflow-hidden flex flex-col",
                hasStarted ? "flex-1 opacity-100" : "flex-[0.001] opacity-0"
            )}>
            ...
            <div className={cn(
                ...
                hasStarted
                    ? "p-3"
                    : "flex flex-col items-center justify-center h-full absolute inset-0 z-10"
            )}>
                <div className={cn(
                    "w-full transition-all duration-700 ease-in-out",
                    hasStarted ? "max-w-full" : "max-w-2xl px-6"
                )}>
                    <form
                    ...
                    >
                        <div className="relative group transition-all duration-700">
                            <GlassInput
                                ...
                                className={cn(
                                    ...
                                    hasStarted
                                        ? "pr-12"
                                        : "pr-14 py-6 text-lg rounded-full bg-black/40 border-white/10 focus:border-rose-500/50 shadow-2xl shadow-black/50 backdrop-blur-xl h-16"
                                )}
                                autoFocus
                            />
                            <LiquidButton
                                ...
                                className={cn(
                                    "absolute top-1/2 -translate-y-1/2 rounded-full shadow-lg transition-all duration-300",
                                    hasStarted
                                        ? "right-1.5 w-8 h-8 p-0 min-h-0 hover:scale-110"
                                        : "right-2 w-10 h-10 p-0 min-h-0 hover:scale-105"
                                )}
                                icon={ArrowRight}
                            >
                            </LiquidButton>
                        ...
                        {/* Helper Text - Fades out when started */}
                        <div className={cn(
                            "text-center mt-4 text-[#606060] text-sm font-light transition-all duration-500 overflow-hidden",
                            hasStarted ? "opacity-0 max-h-0 mt-0" : "opacity-100 max-h-10"
                        )}>
